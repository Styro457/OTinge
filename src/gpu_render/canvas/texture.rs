...
pub fn create_fake_document() -> Document {
    let mut document = Document::new(500, 500, Box::new(RGBAf32Buffer::new(Pos2::ZERO)));
    let mut layer1 = Layer::new("Layer 1", None);
    let solid_red_painter = Box::new(SolidPainter::new(&document,
        Pos2::new(500, 500),
        Color::RGBAf32(1.0, 0.0, 0.0, 1.0),
    ));
    layer1.painter = Some(solid_red_painter);
    let mut layer2 = Layer::new("Layer 2", None);
    let solid_green_painter = Box::new(SolidPainter::new(&document,
        Pos2::new(300, 300),
        Color::RGBAf32(0.0, 1.0, 0.0, 0.5),
    ));
